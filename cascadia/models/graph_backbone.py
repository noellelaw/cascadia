...
class GraphBackbone(nn.Module):
    """Graph-based backbone generation for flood extents.

    GraphBackbone parameterizes a generative model over spatial flood fields,
    using a graph-based representation of the underlying terrain, infrastructure, 
    and conditioning inputs.

    Args:
        See documention of `layers.structure.flood_graph.FloodFeatureGraph`,
    ...
    Inputs:
        X (Tensor): Backbone values (e.g. depths / elevations) with shape
                `(num_batch, height, width)`.
        C (LongTensor): Global conditioning (e.g., SLR metadata, climate driver) with shape `(num_batch, D)` 
                or a Spatial or categorical conditioning map (e.g., land use, elevation class) 
            with shape `(num_batch, height, width)`.
    ...
    def forward(
        self,
        X: torch.Tensor,  # shape: (B, H, W)
        C: torch.Tensor,  # shape: (B, H, W) or (B, num_conditions)
        t: Optional[Union[torch.Tensor, float]] = None,
        **kwargs,
    ):
        """
        Forward pass for flood extent using DDPM + GNN.
        X: Flood depth grids (B, H, W)
        C: Conditioning inputs (e.g., land use, elevation) or global metadata
        t: Diffusion timestep(s)
        """

        assert X.ndim == 3, f"Expected input X of shape (B, H, W), got {X.shape}"

        # Sample noised flood depth grid
        if t is None:
            X_t, t = self.noise_perturb(X, C)
        else:
            X_t = self.noise_perturb(X, C, t=t)

        # Predict denoised flood grid + optional geometric outputs (if using GNN + flow edges)
        ...
        if C.ndim == 3:
            weights = (C > 0).float().sum(dim=(1, 2))  # (B,)
        else:
            weights = torch.ones(X.size(0), device=X.device)

        # Importance weighting using noise schedule (SSNR)
        ssnr = self.noise_perturb.noise_schedule.SSNR(t)
        prob_ssnr = self.noise_perturb.noise_schedule.prob_SSNR(ssnr)
        importance_weights = 1.0 / prob_ssnr

        _importance_weight = lambda h: h * importance_weights.reshape([-1] + [1] * (h.ndim - 1))
        _weighted_avg = lambda h: (weights * _importance_weight(h)).sum() / (weights.sum() + self._loss_eps)

        # If GNN used to predict geometric flow transforms (e.g., intercell motion)
        ...
        # Convert ELBO to neg_elbo (convention)(lol)
        losses_extend = {}
